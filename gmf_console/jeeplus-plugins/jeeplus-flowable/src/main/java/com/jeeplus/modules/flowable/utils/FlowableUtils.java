...
//		groupEntity.setType(role.getRoleType());
	...
	public static String toTimeString(long time) {
		...
		if (day > 0){
			sb.append(day).append(DictUtils.getLanguageLabel("天", ""));
		}
		if (hour > 0){
			sb.append(hour).append(DictUtils.getLanguageLabel("时", ""));
		}
		if (minute > 0){
			sb.append(minute).append(DictUtils.getLanguageLabel("分", ""));
		}
		if (second > 0){
			sb.append(second).append(DictUtils.getLanguageLabel("秒", ""));
		}
		return sb.toString();
	}
