<template name='sunui-upimg'>
	<view>
		<view class="sunsin_picture_list">
			...
			<view>
				<view class='sunsin_picture_item sunsin_width' v-if="(upImgConfig.iconReplace =='' || upImgConfig.iconReplace==undefined) && upload_picture_list.length < upImgConfig.count || upImgConfig.notli" >
				...
				<view class='sunsin_picture_item sunsin_width' v-show="upload_picture_list.length < upImgConfig.count || upImgConfig.notli" v-else>
...
<script>
	...
	export default {
		...
		onLoad() {
			console.log("onLoad():upImgConfig：",this.upImgConfig)
		}
	...
	const upload_file_server = async (_this, configs, upload_picture_list, j) => {
		console.log("进入upload_file_server方法:",configs)
		// 腾讯云配置
		let cosConfig = {
		...
		let Key = "/epin/idcard/"+filePath.substr(filePath.lastIndexOf('/') + 1); // 这里指定上传的文件名
	...
	const cImage = (_this, count, configs) => {
		...
		// _this.upImgConfig=configs
		console.log("_this.upImgConfig",_this.upImgConfig)
		...
		uni.chooseImage({
			...
			success: async (res) => {
				for (let i = 0, len = res.tempFiles.length; i < len; i++) {
					...
					_this.upload_picture_list.length > config.count ? _this.upload_picture_list = _this.upload_picture_list.slice(0,config.count) : '';
					console.log("_this.upload_picture_list: ",_this.upload_picture_list)
					console.log("config.count: ",config.count)
					console.log("upImgConfig: ",_this.upImgConfig)
				}
				console.log("_this.upload_picture_list.length:",_this.upload_picture_list.length)
				// 过滤多出的预览图片
