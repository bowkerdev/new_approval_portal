<template lang="html">
  <div :val="value_">
    <div>
      <el-radio v-model="type" label="1" size="mini" border>{{$i18n.t('每周')}}</el-radio>
    </div>
    <div>
      <el-radio v-model="type" label="5" size="mini" border>{{$i18n.t('不指定')}}</el-radio>
    </div>
    <div>
      <el-radio v-model="type" label="2" size="mini" border>{{$i18n.t('周期')}}</el-radio>
      <span style="margin-left: 10px; margin-right: 5px;">{{$i18n.t('从星期')}}</span>
      <el-input-number @change="type = '2'" v-model="cycle.start" :min="1" :max="7" size="mini" style="width: 100px;"></el-input-number>
      <span style="margin-left: 5px; margin-right: 5px;">{{$i18n.t('至星期')}}</span>
    ...
    <div>
      ...
      <span style="margin-left: 5px; margin-right: 5px;">{{$i18n.t('开始，每')}}</span>
      <el-input-number @change="type = '3'" v-model="loop.end" :min="1" :max="7" size="mini" style="width: 100px;"></el-input-number>{{$i18n.t('天执行一次')}}</div>
    <div>
      <el-radio v-model="type" label="7" size="mini" border>{{$i18n.t('指定周')}}</el-radio>
      <span style="margin-left: 10px; margin-right: 5px;">{{$i18n.t('本月第')}}</span>
      <el-input-number @change="type = '7'" v-model="week.start" :min="1" :max="4" size="mini" style="width: 100px;"></el-input-number>
      <span style="margin-left: 5px; margin-right: 5px;">{{$i18n.t('周，星期')}}</span>
      <el-input-number @change="type = '7'" v-model="week.end" :min="1" :max="7" size="mini" style="width: 100px;"></el-input-number>
    </div>
    <div>
      <el-radio v-model="type" label="6" size="mini" border>{{$i18n.t('本月最后一个')}}</el-radio>
      <span style="margin-left: 10px; margin-right: 5px;">{{$i18n.t('星期')}}</span>
