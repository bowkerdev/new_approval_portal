  ...
  export default {
    ...
    methods: {
      ...
      resetDocumentClientHeight () {
        ...
        window.onresize = () => {
          this.documentClientHeight = document.documentElement['clientHeight']
          // console.log('当前默认菜单类型：', this.$refs.navbar.defaultLayout)
          let _defaultLayout = this.$refs.navbar.defaultLayout
          if (_defaultLayout !== undefined && _defaultLayout === 'top') {
            this.$refs.navbar.fixTopMenu()  // 修复 重置窗口左侧菜单变横向菜单bug 【占锐 2020年3月20日15:56:05】
          }
        }
      },
      showRight (flag) {
