<template>
  <div>
      <el-form :inline="true" v-show="isSearchCollapse" class="query-form" ref="searchForm" :model="searchForm" @keyup.enter.native="refreshList()" @submit.native.prevent>
            <!-- 搜索框-->
		     ...
		     <el-form-item prop="code">
                <el-input size="small" v-model="searchForm.code" :placeholder="$i18n.t('编码')" clearable></el-input>
		     </el-form-item>
          <el-form-item>
      ...
      <el-row>
        <el-button v-if="hasPermission('extension:button:add')" type="primary" size="small" icon="el-icon-plus" @click="add()">{{$i18n.t('新建')}}</el-button>
    ...
    <el-table
      ...
      <el-table-column
        ...
        :label="$i18n.t('操作')">
        <template  slot-scope="scope">
          <el-button v-if="hasPermission('extension:button:view')" type="text" icon="el-icon-view" size="mini" @click="view(scope.row.id)">{{$i18n.t('查看')}}</el-button>
          <el-button v-if="hasPermission('extension:button:edit')" type="text" icon="el-icon-edit" size="mini" @click="edit(scope.row.id)">{{$i18n.t('修改')}}</el-button>
          <el-button v-if="hasPermission('extension:button:del')" type="text" size="mini" icon="el-icon-delete" @click="del(scope.row.id)">{{$i18n.t('删除')}}</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
