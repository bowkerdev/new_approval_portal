  ...
  <el-dialog
    ...
    <el-form :model="inputForm" ref="inputForm" :class="method==='view'?'readonly':''" :disabled="method==='view'" v-loading="loading" @keyup.enter.native="doSubmit()"
      ...
      <el-row  :gutter="15">
        ...
        <el-col :span="24">
            <el-form-item :label="$i18n.t('监听器类型')" prop="listenerType"
                :rules="[
                 {required: true, message:'监听器类型不能为空', trigger:'blur'}
                 ]">
                    <el-radio-group v-model="inputForm.listenerType">
                          <el-radio v-model="listenerType" label="1">{{$i18n.t('执行监听器')}}</el-radio>
                          <el-radio v-model="listenerType" label="2">{{$i18n.t('任务监听器')}}</el-radio>
                    </el-radio-group>
	         </el-form-item>
        </el-col>
        <el-col :span="24">
            <el-form-item :label="$i18n.t('事件')" prop="event"
                :rules="[
                 {required: true, message:'事件不能为空', trigger:'blur'}
                 ]">
		            <el-select v-model="inputForm.event" :placeholder="$i18n.t('请选择')"  style="width: 100%;">
                          <el-option
        ...
        <el-col :span="24">
            <el-form-item :label="$i18n.t('值类型')" prop="valueType"
                :rules="[
                 {required: true, message:'值类型不能为空', trigger:'blur'}
                 ]">
                    <el-radio-group v-model="inputForm.valueType">
                        <el-radio v-model="valueType" label="1">{{$i18n.t('类')}}</el-radio>
                        <el-radio v-model="valueType" label="2">{{$i18n.t('表达式')}}</el-radio>
                        <el-radio v-model="valueType" label="3">{{$i18n.t('委托表达式')}}</el-radio>
                    </el-radio-group>
	         </el-form-item>
        </el-col>
        <el-col :span="24">
            <el-form-item :label="valueLabel" prop="value"
                :rules="[
                 {required: true, message:'值不能为空', trigger:'blur'}
			        ...
			        <el-input v-model="inputForm.value" :placeholder="$i18n.t('请填写值')"     ></el-input>
	         </el-form-item>
        </el-col>
        </el-row>
    </el-form>
