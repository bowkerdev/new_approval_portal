  ...
  <div>
      <el-form :inline="true" v-show="isSearchCollapse" class="query-form" ref="searchForm" :model="searchForm" @keyup.enter.native="refreshList()" @submit.native.prevent>
            <!-- 搜索框-->
		     <el-form-item prop="version">
                <el-input size="small" v-model="searchForm.version" :placeholder="$i18n.t('版本号')" clearable></el-input>
		     </el-form-item>
		     <el-form-item prop="status">
                <el-input size="small" v-model="searchForm.status" :placeholder="$i18n.t('状态')" clearable></el-input>
		     </el-form-item>
		     <el-form-item prop="isPrimary">
                <el-input size="small" v-model="searchForm.isPrimary" :placeholder="$i18n.t('是否主版本')" clearable></el-input>
		     </el-form-item>
          <el-form-item>
    ...
    <el-table
	  ...
	  <el-table-column
        ...
        :label="$i18n.t('状态')">
        <template slot-scope="scope">
          <el-tag v-if="scope.row.status === '1'" size="small" type="success">{{$i18n.t('已发布')}}</el-tag>
	  ...
	  <el-table-column
        ...
        :label="$i18n.t('是否主版本')">
          <template slot-scope="scope">
          <el-tag v-if="scope.row.isPrimary === '1'" size="small" type="success">{{$i18n.t('主版本')}}</el-tag>
      ...
      <el-table-column
        ...
        :label="$i18n.t('操作')">
        <template  slot-scope="scope">
          <el-button type="text" icon="el-icon-view" size="small" @click="view(scope.row.id)">{{$i18n.t('预览')}}</el-button>
          <el-button type="text" icon="el-icon-edit"  v-if="scope.row.isPrimary === '0'" size="small" @click="updatePrimary(scope.row.id)">{{$i18n.t('设置为主版本')}}</el-button>
          <el-button type="text"  icon="el-icon-delete"  v-if="scope.row.isPrimary === '0'" size="small" @click="del(scope.row.id)">{{$i18n.t('删除')}}</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
