  ...
  <div>
    ...
    <el-tabs type="border-card" v-model="taskSelectedTab">
      <el-tab-pane label="表单信息" name="form-first">
        <component :formReadOnly="formReadOnly" v-if="formType === '2'" :class="formReadOnly?'readonly':''" ref="form"
          :businessId="businessId" :is="form"></component>

        <PreviewForm v-if="formType !== '2'" :processDefinitionId="procDefId" :edit="true" :taskFormData="taskFormData"
          ref="form" />
      </el-tab-pane>
      <el-tab-pane label="流程信息" v-if="procInsId" name="form-second">
        <el-card class="box-card" shadow="hover">
          ...
          <el-timeline :reverse="true" v-if="histoicFlowList.length">
            <el-timeline-item color="#3f9eff" :key="index" v-for="(act, index) in histoicFlowList" :timestamp="moment(act.histIns.endTime).format('YYYY-MM-DD')"
              placement="top">
              <el-card>
      ...
      <el-tab-pane label="流转记录" v-if="procInsId" name="form-forth">
        <el-card class="box-card" shadow="hover" style="margin-top:5px">
          ...
          <el-steps :active="histoicFlowList.length">
            <el-step :key="index" v-for="(act, index) in histoicFlowList" :title="act.histIns.activityName"
              finish-status="success" :description="(act.assigneeName||'') +'    '   + moment(act.histIns.endTime).format('YYYY-MM-DD HH:mm:ss')"></el-step>
          </el-steps>
          <el-table border :data="histoicFlowList" style="width: 100%">
      ...
      <el-form :model="auditForm" ref="auditForm" label-width="120px">
        <el-col :span="16">
          ...
          <el-form-item v-if="taskId" label="审批信息" prop="comment">
            <el-input type="textarea" :rows="3" placeholder="请输入审批意见" v-model="auditForm.comment">
            </el-input>
          </el-form-item>
        </el-col>
        <el-col :span="16">
        ...
        <el-col :span="16">
          <el-form-item>
            <el-button type="primary" v-show="button.isHide === '0'" v-for="(button, index) in buttons" :key="index"
              @click="submit(button, buttons)" v-noMoreClick>{{button.name}}</el-button>
          </el-form-item>
        </el-col>
      </el-form>
    </el-card>
...
<script>
  ...
  export default {
    activated() {
      ...
      // 读取历史任务列表
      this.$http.get(`/flowable/task/histoicFlowList?procInsId=${this.procInsId}`).then(({
        data
      }) => {
    ...
    methods: {
      init() {
        ...
        this.formReadOnly = this.$route.query.formReadOnly !== undefined && this.$route.query.formReadOnly !== 'false' &&
      ...
      submit(currentBtn, buttons) {
        ...
        if (currentBtn.id === 'start') {
          ...
          if (this.formType === '2') {
            ...
            this.$refs.form.submitTaskFormData(vars, this.procInsId, this.taskId, this.auditForm.assign, this.auditForm
              .comment, (data) => {
                if (data.success) {
                  this.$events.$emit('closeTab', this.$route.fullPath)
                  this.$router.push('/flowable/task/TodoList')
