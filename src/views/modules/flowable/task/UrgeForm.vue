<template>
  <el-dialog
    ...
    <el-form ref="inputForm" :model="inputForm"   v-loading="loading" @keyup.enter.native="inputFormSubmit()" label-width="120px" @submit.native.prevent> 
       <el-form-item :label="$i18n.t('催办方式')" prop="checkedUrgeTypes"  :rules="[
      ...
      <el-form-item :label="$i18n.t('催办内容')" prop="content"  :rules="[
                  {required: true, message:'催办内容不能为空', trigger:'blur'}
                 ]">
            <el-input type="textarea" v-model="inputForm.content" :rows="4" :placeholder="$i18n.t('请输入催办内容')"></el-input>
      </el-form-item>
    </el-form>
