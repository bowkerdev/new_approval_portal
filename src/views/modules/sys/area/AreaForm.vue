<template>
  <el-dialog
    :title="title"
    :close-on-click-modal="false"
     v-dialogDrag
    :visible.sync="visible">
    <el-form size="small" :model="inputForm" :rules="dataRule" ref="inputForm" @keyup.enter.native="doSubmit()"
             label-width="80px" v-loading="loading" :class="method==='view'?'readonly':''" :disabled="method==='view'" @submit.native.prevent>
      <el-row  :gutter="15">
        <el-col :span="12">
          <el-form-item :label="$i18nMy.t('上级区域')" prop="parent.id">
             <SelectTree 
             ref="parentTree"
             :props="{
                value: 'id',             // ID字段名
                label: 'name',         // 显示名称
                children: 'children'    // 子级字段名
              }"
            v-if="visible"
            :url="`/sys/area/treeData?extId=${inputForm.id}`"
            :value="inputForm.parent.id"
            :clearable="true" 
            :accordion="true"
            @getValue="(value) => {inputForm.parent.id=value}"/>
          </el-form-item>
        </el-col>
         <el-col :span="12">
            <el-form-item :label="$i18nMy.t('区域名称')" prop="name">
              <el-input maxlength="50" v-model="inputForm.name" :placeholder="$i18nMy.t('区域名称')"></el-input>
            </el-form-item>
        </el-col>
     
        <el-col :span="12">
          <el-form-item :label="$i18nMy.t('区域编码')" prop="code">
            <el-input maxlength="50" v-model="inputForm.code" :placeholder="$i18nMy.t('区域编码')"></el-input>
          </el-form-item>
        </el-col>
        <el-col :span="12">
          <el-form-item :label="$i18nMy.t('区域类型')" prop="type">
            <el-select v-model="inputForm.type" :placeholder="$i18nMy.t('请选择')"  style="width: 100%;">
                <el-option
                  v-for="item in this.$dictUtils.getDictList('sys_area_type')"
                  :key="item.value"
                  :label="item.label"
                  :value="item.value">
                </el-option>
            </el-select>
          </el-form-item>
        </el-col>
        <el-col :span="12">
           <el-form-item :label="$i18nMy.t('排序号')" prop="sort">
            <el-input-number  v-model="inputForm.sort" :step="30" style="width:100%"></el-input-number>
          </el-form-item>
        </el-col>
        <el-col :span="12">
          <el-form-item :label="$i18nMy.t('备注')" prop="remarks">
            <el-input maxlength="200" v-model="inputForm.remarks" type="textarea"
                  :rows="2" :placeholder="$i18nMy.t('备注')"></el-input>
          </el-form-item>
        </el-col>
      
      </el-row>
    </el-form>
    <span slot="footer" class="dialog-footer">
      <el-button @click="visible = false">{{$i18nMy.t('关闭')}}</el-button>
      <el-button v-if="method != 'view'" type="primary" @click="doSubmit()" v-noMoreClick>{{$i18nMy.t('确定')}}</el-button>
    </span>
  </el-dialog>
</template>

<script>
  import SelectTree from '@/components/treeSelect/treeSelect.vue'
  export default {
    data () {
      return {
        areaListTreeProps: {
          label: 'name',
          children: 'children'
        },
        visible: false,
        loading: false,
        method: '',
        title: '新增',
        inputForm: {
          id: '',
          name: '',
          parent: {
            id: '',
            name: ''
          },
          sort: '30',
          code: '', // 区域编码
          type: '', // 区域类型（1：公司；2：部门；3：小组）
          remarks: ''
        },
        dataRule: {
          name: [
            {required: true, message: $i18nMy.t('名称不能为空'), trigger: 'blur'}
          ],
          type: [
            {required: true, message: $i18nMy.t('区域类型不能为空'), trigger: 'blur'}
          ],
          code: [
            {required: true, message: $i18nMy.t('区域编码不能为空'), trigger: 'blur'}
          ]
        }
      }
    },
    components: {
      SelectTree
    },
    methods: {
      init (method, obj) {
        this.inputForm.id = obj.id
        this.method = method
        if (method === 'add') {
          this.title = $i18nMy.t('新增区域')
        } else if (method === 'addChild') {
          this.title = $i18nMy.t('添加下级区域')
        } else if (method === 'edit') {
          this.title = $i18nMy.t('修改区域')
        } else if (method === 'view') {
          this.title = $i18nMy.t('查看区域')
        }
        this.visible = true
        this.$nextTick(() => {
          this.$refs['inputForm'].resetFields()
          this.inputForm.parent.id = obj.parent.id
          this.inputForm.parent.name = obj.parent.name
          if (method === 'edit' || method === 'view') { // 修改或者查看
            this.$http({
              url: `/sys/area/queryById?id=${this.inputForm.id}`,
              method: 'get'
            }).then(({data}) => {
              this.inputForm = this.recover(this.inputForm, data.area)
            })
          }
        })
      },
      // 表单提交
      doSubmit () {
        this.$refs['inputForm'].validate((valid) => {
          if (valid) {
            this.loading = true
            this.$http({
              url: `/sys/area/save`,
              method: 'post',
              data: this.inputForm
            }).then(({data}) => {
              this.loading = false
              if (data && data.success) {
                this.$message({
                  message: $i18nMy.t('操作成功'),
                  type: 'success',
                  duration: 1500
                })
                this.visible = false
                this.$emit('refreshDataList')
              }
            })
          }
        })
      }
    }
  }
</script>