  ...
  <div>
      ...
      <el-form :inline="true" v-show="isSearchCollapse" class="query-form" ref="searchForm" :model="searchForm" @keyup.enter.native="refreshList()" @submit.native.prevent>
         <el-form-item prop="type">
          <el-input size="small" v-model="searchForm.type" :placeholder="$i18n.t('类型')" clearable></el-input>
        </el-form-item>
      <el-form-item>
      ...
      <el-row>
        <el-button v-if="hasPermission('sys:dict:add')" type="primary" size="small" icon="el-icon-plus" @click="add()">{{$i18n.t('新建')}}</el-button>
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
             <el-button v-if="hasPermission('sys:dict:view')" type="text" size="small"
                      @click="view(scope.row.id)">{{$i18n.t('查看')}}</el-button>
            <el-divider direction="vertical"></el-divider>
            <el-button v-if="hasPermission('sys:dict:edit')" type="text" size="small"
                      @click="edit(scope.row.id)">{{$i18n.t('修改')}}</el-button>
            <el-divider direction="vertical"></el-divider>
            <el-button v-if="hasPermission('sys:dict:del')" type="text" size="small" @click="del(scope.row.id)">{{$i18n.t('删除')}}</el-button>
            <el-divider direction="vertical"></el-divider>
            <el-button v-if="hasPermission('sys:dict:edit')" type="text" size="small" @click="showRight(scope.row)">{{$i18n.t('管理键值')}}</el-button>
          </template>
        </el-table-column>
      </el-table>
    <el-pagination
