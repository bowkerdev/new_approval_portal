...
<div>
  <el-dialog
    ...
    <el-form size="small" :model="inputForm" ref="inputForm" v-loading="loading" :class="method==='view'?'readonly':''"  :disabled="method==='view'"
      ...
      <el-row  :gutter="15">
        <el-col :span="12">
            <el-form-item :label="$i18nMy.t('岗位名称')" prop="name"
                :rules="[
                  {required: true, message:'岗位名称不能为空', trigger:'blur'}
                 ]">
              <el-input v-model="inputForm.name" :placeholder="$i18nMy.t('请填写岗位名称')"     ></el-input>
           </el-form-item>
        </el-col>
        <el-col :span="12">
            <el-form-item :label="$i18nMy.t('岗位编码')" prop="code"
                :rules="[
                  {required: true, message:'岗位编码不能为空', trigger:'blur'},
                  {validator: validator.isRightfulString, trigger:'blur'}
                 ]">
              <el-input v-model="inputForm.code" :placeholder="$i18nMy.t('请填写岗位编码')"     ></el-input>
           </el-form-item>
        </el-col>
        <el-col :span="12">
            <el-form-item :label="$i18nMy.t('岗位类型')" prop="type"
                :rules="[
        ...
        <el-col :span="12">
            <el-form-item :label="$i18nMy.t('是否可用')" prop="status"
                :rules="[
        ...
        <el-col :span="12">
            <el-form-item :label="$i18nMy.t('岗位排序')" prop="sort"
                :rules="[
  ...
  export default {
    ...
    methods: {
      init (method, id) {
        ...
        if (method === 'add') {
          this.title = $i18nMy.t('新建岗位')
        } else if (method === 'edit') {
          this.title = $i18nMy.t('修改岗位')
        } else if (method === 'view') {
          this.title = $i18nMy.t('查看岗位')
        }
        this.visible = true
        this.loading = false
        this.$nextTick(() => {
