...
<el-tabs tab-position="left">
    <el-tab-pane :label="$i18n.t('基本信息')">
      <h3 style="margin-left:40px">{{$i18n.t('基本信息')}}</h3>
      <el-form :model="generalForm" ref="generalForm" label-width="100px">
        <el-form-item :label="$i18n.t('姓名')" :rules="[ { required: true, message: '必填项不能为空', trigger: 'blur' }]" prop="name">
          <el-input v-model="generalForm.name"></el-input>
        </el-form-item>
         <el-form-item :label="$i18n.t('签名')" prop="sign">
          <el-input v-model="generalForm.sign"></el-input>
        </el-form-item>
         <el-form-item :label="$i18n.t('公司')" prop="company.name">
          <el-input v-model="generalForm.company.name" :disabled="true"></el-input>
        </el-form-item>
        <el-form-item :label="$i18n.t('部门')" prop="office.name">
        ...
        <el-form-item>
          <el-button type="primary" @click="submitGeneralForm()">{{$i18n.t('保存')}}</el-button>
        </el-form-item>
      </el-form>
    </el-tab-pane>
    <el-tab-pane :label="$i18n.t('联系方式')">
      <h3 style="margin-left:40px">{{$i18n.t('联系方式')}}</h3>
      <el-form :model="concatForm" ref="concatForm" label-width="100px">
        <el-form-item :label="$i18n.t('邮箱')" :rules="[{type:'email', message:'请输入正确的邮箱地址', trigger:'blur'}]" prop="email">
          <el-input v-model="concatForm.email"></el-input>
        </el-form-item>
         <el-form-item :label="$i18n.t('手机')" prop="mobile" :rules="[{validator:validator.isMobile, trigger:'blur'}]">
          <el-input v-model="concatForm.mobile"></el-input>
        </el-form-item>
         <el-form-item :label="$i18n.t('电话')" prop="phone" :rules="[{validator:validator.isPhone, trigger:'blur'}]">
          <el-input v-model="concatForm.phone"></el-input>
        </el-form-item>
        <el-form-item>
          <el-button type="primary" @click="submitConcatForm()">{{$i18n.t('保存')}}</el-button>
        </el-form-item>
      </el-form>
    </el-tab-pane>
    <el-tab-pane :label="$i18n.t('我的头像')">
      <h3 style="margin-left:40px">{{$i18n.t('我的头像')}}</h3>
      <el-upload
    ...
    <el-tab-pane :label="$i18n.t('修改密码')">
      <h3 style="margin-left:40px">{{$i18n.t('修改密码')}}</h3>
      <el-form :model="pwdForm" ref="pwdForm" label-width="100px">
        <el-form-item :label="$i18n.t('旧密码')" :rules="[ { required: true, message: '必填项不能为空', trigger: 'blur' }]" prop="oldPassword">
          <el-input v-model="pwdForm.oldPassword"></el-input>
        </el-form-item>
         <el-form-item :label="$i18n.t('新密码')" :rules="[ { required: true, message: '必填项不能为空', trigger: 'blur' }]" prop="newPassword">
          <el-input v-model="pwdForm.newPassword"></el-input>
        </el-form-item>
         <el-form-item :label="$i18n.t('确认新密码')" :rules="rule" prop="confirmNewPassword">
          <el-input v-model="pwdForm.confirmNewPassword"></el-input>
        </el-form-item>
        <el-form-item>
          <el-button type="primary" @click="submitPwdForm()">{{$i18n.t('保存')}}</el-button>
        </el-form-item>
      </el-form>
    </el-tab-pane>
    <el-tab-pane :label="$i18n.t('我的日志')">
